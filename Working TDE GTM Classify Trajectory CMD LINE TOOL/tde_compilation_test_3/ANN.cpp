...
{
	//std::cout << "inside annPrintPt" << std::endl;

	for (int j = 0; j < dim; j++) {
		//std::cout << "inside for inside annPrintPt" << std::endl;

		//std::cout << pt[j] << std::endl;
		//std::cout << "after cout for pt inside annPrintPrint" << std::endl;

		fprintf(out, FLOAT_OUT, pt[j]); //out << pt[j];

		//std::cout << "before if inside for inside annPrintPt" << std::endl;

		if (j < dim-1) fprintf(out, " "); // out << " ";
	}

	//std::cout << "after for loop inside annPrintPt" << std::endl;
}
...
ANNpointArray annAllocPts(int n, int dim)		// allocate n pts in dim
{
	...
	//cout << "after pa pointArray" << endl;
	// allocate points
	ANNpoint p  = new ANNcoord[n*dim];
	
	//cout << "after points allocation" << endl;
