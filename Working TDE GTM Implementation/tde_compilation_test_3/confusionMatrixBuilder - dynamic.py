...
#A script designed to compute confusion matrices from any folder full of classification results.
#This script was specifically designed for use with Jordan Frank's GTM classification results.
#Donald Buhl-Brown

#Used when you have multiple models of the same class
numberOfDuplicateClassModels = int( raw_input( "How many duplicate models are there if none put 0: " ) )
  ...
  def SetupClassificationResults( self ):
    ...
    for item in self.dataFileNamesList:
      ...
      for line in classificationResultFile.readlines( ):
        ...
        scoresIndex = 0
        similarityScoresIndex = 0
      
      
        scores = line.split( " ", len( self.annotationList ) * ( numberOfDuplicateClassModels + 1 ) ) #Replace 3 with the number of possibilities
        ...
        for item in scores:
          ...
          if( scoresIndex >= 10 and numberOfDuplicateClassModels != 0 ):
        
            similarityScoresIndex = scoresIndex % len( self.annotationList )
        
          try:
            
            tmpScores = scores[ scoresIndex ]
            
            #Added this to deal with R code outputting NaN
            if tmpScores == "NaN":
              tmpScores = 0
            
            self.similarityScoreList[ similarityScoresIndex ] = self.similarityScoreList[ similarityScoresIndex ] + float( scores[ scoresIndex ] )
          
            #print similarityScoresIndex
            #print scoresIndex
            #raw_input( )
            ...
            print len( scores )
            self.similarityScoreList[ similarityScoresIndex ] = self.similarityScoreList[ similarityScoresIndex ] + 0
      ...
      highestScoreIndex = 0
      highestScore = self.similarityScoreList[ 0 ]
